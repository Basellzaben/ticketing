    ...
    return Stack(children: <Widget>[
      ...
      Scaffold(
          ...
          appBar: AppBar(
            ...
            bottomOpacity: 800.0,
            elevation: 4.0,
            title: Widgets.Appbar(context,LanguageProvider.Llanguage('dailywork')
                                    ...
                                    SizedBox(
                                      ...
                                      child: FutureBuilder(
                                        ...
                                        builder: (
                                            ...
                                            AsyncSnapshot<List<
                                                TicketsModel>> snapshot) {
                                          if (snapshot.hasData) {
                                            List<TicketsModel>? Visits = snapshot
                                                .data;
                                            return ListView(
  ...
  CloseTicket( BuildContext context,String note,String ticketid) async {
    ...
    showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(l.getLanguage()=="AR"?'جار اغلاق التذكره ...':'close ticket..'),
        ));
    ...
    try {
      ...
      if (jsonResponse.toString().contains('1S')  ) {
        ...
        showDialog(
            ...
            builder: (_) => AlertDialog(
              title: Text(l.Llanguage('ClosTicket')),
              content: Text(l.getLanguage()=="AR"?'تم اغلاق التذكره بنجاح':'close ticketing is done..'),
            ));
...
"hgvj"            ),
                      ...
                      if (OpenTickets!.isNotEmpty) {
                        return Center(
                          child: Container(
                            decoration: BoxDecoration(
                              border: Border(
                                bottom: BorderSide(width: 0.5, color: HexColor(ThemP.color)),
                                top: BorderSide(width: 0.5, color: HexColor(ThemP.color)),
                                left: BorderSide(width: 0.5, color: HexColor(ThemP.color)),
                                right: BorderSide(width: 0.5, color: HexColor(ThemP.color)),
  ...
  AddDailyWork( BuildContext context,String note,String ticketid,String worktime) async {
    var l = Provider.of<Language>(context, listen: false);
    var Loginprovider = Provider.of<LoginProvider>(context, listen: false);
    DateTime now = DateTime.now();
    ...
    showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(l.getLanguage()=="AR"?'اضافه العمل اليومي ...':'send daily work..'),
        ));
    ...
    try {
      ...
      if (jsonResponse.toString().contains('1S')  ) {
        showDialog(
            ...
            builder: (_) => AlertDialog(
              title: Text(l.Llanguage('dailywork')),
              content: Text(l.getLanguage()=="AR"?'تم اضافه العمل اليومي':'send daily work is done..'),
            ));
